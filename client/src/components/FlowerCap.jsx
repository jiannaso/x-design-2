...
const CapPetal = ({ position, rotation, bevelThickness, bevelSize}) => {
    ...
    const extrudeSettings = {
        ...
        depth: 0.02,
        bevelEnabled: true,
        bevelThickness: 0.3 + bevelThickness / 50,
        bevelSize: 0.4 + bevelSize/ 100,
        bevelOffset: 0,
        bevelSegments: 100
    };
...
const FlowerCap = ({ posx=0, posy=0, posz=0, numPetals = 8, petalRotation = 33, bevelThickness = 0.4, bevelSize = 0.4}) => {
    ...
    const petals = useMemo(() => {
        return Array.from({ length: numPetals }, (_, i) => {
            ...
            const rotation = [Math.PI / 1, angle, 3 + petalRotation / 50 ]; 
            return <CapPetal key={i} position={[x + posx, elevation, z + posz]} rotation={rotation} bevelThickness={bevelThickness} bevelSize={bevelSize}/>;
        });
    }, [numPetals, elevation]);
...
// DONE 1. bevelThickness -> from 0.3-4. 
// DONE 2. bevelSize -> 0.4 to 1. 
// DONE 3. in const petal function, const rotation = [Math.PI / 1, angle, 4]; the number 4 can be 3-4-5. these three numbers alter the shape really well! 
// DONE 4. also numpetals can change from 3-4 to 20! 
